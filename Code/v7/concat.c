...
void affichage_dynamique(unsigned long long int numberofbackbone, Hasht *h) {
	...
	if (!(numberofbackbone & lemodulo)) {
		...
		prettyprint2(stdout,h->insertions - h->elements + h->nombre_map,"folded maps   ");
		prettyprint2(stdout,h->nombre_map,"uniq maps");
		fflush(stdout); 
	}
}
		...
		for(j=0; j < vertices[i].degree; j++)
	  	{ 
	    	if (non_isomorph(vertices[i].edges,vertices[i].degree,j))//optimization: do not consider two edges which are equivalent in their neighborood for the first vertex
	    	{
				...
				map->edgenumber = vertices[i].degree - mapsize + 2 +1;
				//-mapsize+2 corresponds to the number of edges not used in the signature
				//+1 corresponds to the size of the signature stored in the first element
