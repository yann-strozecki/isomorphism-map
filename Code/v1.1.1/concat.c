...
{//the id should be in the map instead of the type: then simplify the condition
  ...
  if(vertices[map->vertexarray[i].type].id > vertices[map->vertexarray[map->vertexnumber-i-1].type].id) return 0;
  return 1;
}
...
Map initializemap(){//create a new map with value -1 on the edges
	...
	for (i=0 ; i<mapsize ; i++) {
		...
		map.signature = malloc(mapsize * maxdegree * sizeof(unsigned int));
		}
	return map;
}
  ...
  if (!connectionstack) { fprintf(stderr,"cannot malloc connectionstack in concat.c\n"); exit(41); }
	...
	for(j=0; j < vertices[i].degree; j++)
	  { 
		...
		map.edgenumber = vertices[i].degree - mapsize + 3;
		currentsize=0; //size -1 of the current path
		connectionstack[0] = 0;
		labelvalues = shift - vertices[i].labelvalue;//on devrait intégrer le shift ici
		...
		while(currentsize >=0)
		  {
		    ...
		    if(currentsize == mapsize-1 &&  (labelvalues==shift) && smallest_representation(&map) ) //produce a complete backbone when the path is of the right size and the path is almost foldable
		      {
...
unsigned long long int generate_trees(int verticesnumber, Hasht *h, Map map, FreeEdge *outline)
{
  ...
  while(1){
    ...
    //printf("\n");
    if(size < mapsize && free_edge_number > 0 &&  almostfoldabletree[size][shift - labelvalues])
