...
typedef struct CompatibleVertices {
	uint16_t *list; //array of the index of vertices which can be connected to some letter by their first edge 
	uint16_t vertexnumber; //size of the list
} CompatibleVertices;
...
typedef struct FreeEdge {  //a free edge in the graph
	...
	uint32_t vertexindex;  //index of the corresponding vertex in the vertexarray of the  Map
	uint32_t edgeindex; //number of the edge in the edges array  
} FreeEdge; //to merge with Edge

// The Map Structure//
typedef struct Edge {
	uint32_t vertexindex;//vertex connected to the edge
	uint32_t edgeindex;//position of this edge in the neighborood of the end vertex
  int val;//partial signature containing (edge label, vertex id)
} Edge; 

typedef struct VertexMap {
  uint32_t degree;//degree of the vertex
  uint32_t type;//position in the list of vertices of type Vertex 
