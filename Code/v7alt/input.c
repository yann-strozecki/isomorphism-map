...
Vertex *create_rotated_vertices(int *size, Vertex *vert){
  ...
  //compute the localtype of each vertex
  for(i=0;i<verticesnumber;i++){
    vert[i].localtype = malloc(sizeof(uint64_t) * vert[i].degree);
    for(j=0;j<vert[i].degree;j++){
      vert[i].localtype[j] = 0;
      for(int k = 0; k < vert[i].degree; k++){
        vert[i].localtype[j] |= vert[i].edges[(j + k) % vert[i].degree] << (4*k);
      } 
    }
  }
  *size = verticesnumber;
