...
unsigned int hash_basic(unsigned int* tab, int size){
...
unsigned int* write_signature_heap(Heap *hp, unsigned int* tab, int size){
...
int insert_hasht(Hasht *h, unsigned int* tab, int size){//return 0 when the element was already there, 1 when we insert it
  unsigned int key = hash_basic(tab,size);
  //printf("Clé %d \n",key);
  h->insertions++; 
  //insertion with a key not yet used
  if(h->hash[key].data == NULL){
    h->hash[key].data = write_signature_heap(h->hp,tab,size);
    h->hash[key].next = NULL;//end the list
    h->elements++;
    return 1;
  }
  elem* current_element =&h->hash[key];
  while(memcmp(tab,current_element->data,size*sizeof(unsigned int))){//the element is different
    if(current_element->next == NULL){ //an element is added at the end of the list
      current_element->next = write_element_heap(h->hp);
      (current_element->next)->data = write_signature_heap(h->hp,tab,size);
      h->elements++;
      return 1; 
    }
...
void fast_insert_hasht(Hasht *h, unsigned int* tab, int size){//optimized version when we know that the signature is not in the hashtable
  unsigned int key = hash_basic(tab,size);
  //printf("Clé %d \n",key);
  h->insertions++; 
  h->elements++;
...
void hash_insert(Hasht* h, Map* M){
  ...
  if(insert_hasht(h, M->signature, mapsize)){
    h->nombre_map++;
    for(int i = edge+1; i < M->vertexarray[vertex].degree ; i++){
      if(vertices[M->vertexarray[vertex].type].edges[i] == rarest_edge_label){
        ...
        fast_insert_hasht(h, M->signature,mapsize);
      }
    }
    for (int i = vertex+1 ; i<mapsize; i++){
