...
int main (int argc, char *argv[])
{
  ...
  int *edge_min = malloc(vertexnumber*sizeof(int)); // Array containing for each vertex type, the fewest represented type of edge
  int *edge_min_nb = malloc(vertexnumber*sizeof(int)); // Array containing the multiplicity of the pair (vertex id, edge_min[vertex id])
  int min_occurence;
  for (i = 0;i < vertexnumber;i++){
    ...
    for (j = 0;j < label.size;j++){
      if (edge_multiplicity[j] != 0 && edge_multiplicity[j] < min_occurence){
        ...
        edge_min[i] = j;
        edge_min_nb[i] = min_occurence;
      }
      edge_multiplicity[j] = 0;
    }
  }
  set_rarest_pair(edge_min,edge_min_nb,vertexnumber);
